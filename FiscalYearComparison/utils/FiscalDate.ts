...
/**
 * Dates are calendar dates; local construction prevents
 * DateOnly values drifting by UTC offset.
 */
export function dateAtMidnight(
  year: number,
  monthIndex: number,
  day: number
): Date {
...
export function getCurrentFiscalYear(
  ...
  const startThisCalendarYear = dateAtMidnight(
    today.getFullYear(),
    startMonth - 1,
    startDay
  );

  return today >= startThisCalendarYear
    ? today.getFullYear()
    : today.getFullYear() - 1;
}
...
export function getFiscalDayNumber(
  today: Date = new Date(),
  startMonth = 4,
  startDay = 1
): number {
  const start = getFiscalYearStart(
    getCurrentFiscalYear(today, startMonth, startDay),
    startMonth,
    startDay
  );

  const currentDate = dateAtMidnight(
    today.getFullYear(),
    today.getMonth(),
    today.getDate()
  );
  ...
  return (
    Math.floor(
      (currentDate.getTime() - start.getTime()) / 86400000
    ) + 1
  );
}
 ...
 * Example:
 * Today = 25-Sep-2026
 * Fiscal year starts = 01-Apr
 *
 * Result:
 * 01-Apr-2026
 */
export function getFYTDStartDate(
  today = new Date(),
  startMonth = 4,
  startDay = 1
): Date {
 ...
 * Current FYTD end.
 *
 * This is simply today's calendar date.
 *
 * Example:
 * Today = 25-Sep-2026
 * Result = 25-Sep-2026
 */
export function getFYTDCurrentEndDate(
  today = new Date()
): Date {
  return dateAtMidnight(
    today.getFullYear(),
 ...
 * 01-Apr-2025
 */
export function getPreviousFYTDStartDate(
  today = new Date(),
  startMonth = 4,
  startDay = 1
): Date {
...
/**
 * Returns the current FYTD and previous FYTD periods.
 *
 * Example when today is 25-Sep-2026:
 *
 * Current:
...
export function getFYTDPeriods(
  ...
  return [
    ...
    },
    {
      ...
      label: `FY${currentYear - 1}`,
      fiscalYear: currentYear - 1,
      start: getPreviousFYTDStartDate(
        today,
        startMonth,
        startDay
      ),
      end: getPreviousFYTDEndDate(
        today,
        startMonth,
        startDay
      )
...
/**
 * Returns Q1 boundary dates for a given fiscal year
 * based on the organization's fiscal calendar config.
 *
 * Q1 spans 3 calendar months from startMonth.
 *
 * Example for April 1:
 ...
 */
export function getFiscalQ1Period(
  fiscalYear: number,
  startMonth = 4,
  startDay = 1
): { start: Date; end: Date } {
  const start = dateAtMidnight(
    fiscalYear,
    startMonth - 1,
    startDay
  );

  // End date is 3 months later minus 1 day.
  //
  // Example:
  // April 1 -> July 1 minus 1 day = June 30
  const end = new Date(
    fiscalYear,
    startMonth - 1 + 3,
    startDay - 1
...
/**
 * Returns both current FY Q1 and previous FY Q1
 * period definitions.
 */
export function getQ1ComparisonPeriods(
  today = new Date(),
  startMonth = 4,
  startDay = 1
): FiscalPeriod[] {
  ...
  const currentQ1 = getFiscalQ1Period(
    currentYear,
    startMonth,
    startDay
  );

  const prevQ1 = getFiscalQ1Period(
    prevYear,
    startMonth,
    startDay
  );
...
export function fiscalMonthIndex(
  date: Date,
  startMonth = 4
): number {
  return (
    (date.getMonth() - (startMonth - 1) + 12) % 12
  );
}

export function fiscalMonthLabel(
  index: number,
  startMonth = 4
): string {
  return new Date(
